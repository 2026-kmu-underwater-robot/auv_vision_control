...
def generate_launch_description():
    ...
    parameter_specs = [
        ...
        ("approach_area_ratio", 0.005, float, "Bounding-box area ratio that stops normal approach."),
        ...
        ("lane_forward_pwm", 1680, int, "Maximum forward PWM during lane and waypoint motion."),
        ("lane_forward_slow_pwm", 1560, int, "Minimum forward PWM during lane interception and waypoint slowdown."),
        ("lane_lookahead_distance_m", 1.0, float, "LOS lookahead distance along the active lane."),
        ("lane_rejoin_cross_track_tolerance_m", 0.25, float, "Cross-track tolerance for dynamic lane reacquisition."),
        ("lane_rejoin_heading_tolerance_rad", 0.2618, float, "Lane-heading tolerance for dynamic reacquisition."),
        ("lane_forward_full_heading_rad", 0.1745, float, "Heading error below which full lane forward PWM is allowed."),
        ("lane_forward_stop_heading_rad", 0.5236, float, "Heading error at which lane forward motion stops."),
        ("lane_transfer_forward_pwm", 1680, int, "Maximum forward PWM while moving between adjacent lanes."),
        ("lane_transfer_dynamic_rejoin", True, bool, "Join the next lane directly with dynamic LOS instead of two stationary yaw alignments."),
        ("lane_end_transition_distance_m", 3.0, float, "Start joining the next lane this far before the current endpoint; zero reaches the endpoint."),
        ("lane_transfer_slowdown_distance_m", 0.75, float, "Distance where inter-lane transfer slowdown begins."),
        ("lane_transfer_heading_tolerance_rad", 0.0873, float, "Yaw tolerance for each inter-lane alignment phase."),
        ("lane_transfer_heading_hold_sec", 0.3, float, "Required stable yaw duration before advancing a transfer phase."),
        ("waypoint_slowdown_distance_m", 0.25, float, "Distance where waypoint forward slowdown begins."),
        ("waypoint_settle_speed_mps", 0.15, float, "Maximum horizontal speed for waypoint arrival."),
        ("waypoint_settle_hold_sec", 0.3, float, "Required settled duration at a waypoint."),
        ...
        ("waypoint_heading_release_rad", 0.3491, float, "Heading error that stops forward motion for realignment."),
        ("waypoint_yaw_kp", 0.8, float, "Waypoint yaw PID proportional gain."),
        ("waypoint_yaw_ki", 0.1, float, "Waypoint yaw PID integral gain."),
        ("waypoint_yaw_kd", 0.02, float, "Waypoint yaw PID derivative gain."),
        ("waypoint_yaw_integral_limit", 0.7, float, "Absolute waypoint yaw integral limit."),
        ("max_waypoint_yaw_delta_pwm", 160, int, "Maximum waypoint yaw deviation from neutral."),
